...
# -*- coding: utf-8 -*-
"""
Reproduce Figure 7 - U-19Pu-10Zr Swelling vs Burnup
====================================================

This script reproduces Figure 7 from the reference paper:
"Kinetics of fission-gas-bubble-nucleated void swelling of the alpha-uranium phase
of irradiated U-Zr and U-Pu-Zr fuel"

Figure 7 shows cavity-calculated unrestrained swelling as a function of fuel
length (burnup) for U-19Pu-10Zr fuel elements irradiated in EBR-II assembly X423,
compared with experimental data.

What this script does:
- Runs simulations for U-19Pu-10Zr at different temperatures (650-800 K)
- Calculates swelling evolution over burnup
- Compares model predictions with experimental data from the paper
- Generates publication-quality plots matching Figure 7

Material parameters (from Table 1 in paper):
- Dislocation density: 2e13 m^-2 (lower than U-10Zr, resulting in lower swelling)
- Bulk nucleation factor (Fnb): 1e-5
- Boundary nucleation factor (Fnf): 1e-5
- Surface energy: 0.5 J/m^2
- Peak swelling temperature: ~750 K (higher than U-10Zr)

Usage:
    python -m gas_swelling.validation.scripts.reproduce_figure7 --help
    python -m gas_swelling.validation.scripts.reproduce_figure7 --output figure7.png
    python -m gas_swelling.validation.scripts.reproduce_figure7 --output figure7.pdf --no-show
...
try:
    ...
    from gas_swelling.validation.datasets import get_figure7_data
    ...
    from validation.datasets import get_figure7_data
...
# U-19Pu-10Zr material parameters from Table 1 in the paper
# Note: Lower dislocation density than U-10Zr (2e13 vs 7e13 m^-2)
U19PU10ZR_PARAMETERS = {
    'dislocation_density': 2e13,  # m^-2 (lower than U-10Zr)
...
def run_u19pu10zr_simulation(
    ...
    """
    Run a simulation for U-19Pu-10Zr material at specified temperature.
    ...
    """
    # Create parameters with U-19Pu-10Zr specific values
    params = create_default_parameters()
    params['temperature'] = temperature
    params['fission_rate'] = fission_rate

    # Apply U-19Pu-10Zr material parameters
    for key, value in U19PU10ZR_PARAMETERS.items():
...
def calculate_burnup(
    ...
    """
    # Approximate conversion: 1 at.% ≈ 1.15e20 fissions/m^3 for U-19Pu-10Zr
...
def plot_figure7(
    ...
    """
    Create Figure 7 reproduction plot.
    ...
    """
    print_section_header("Creating Figure 7 Reproduction Plot")
    ...
    for data_point in experimental_data:
        if data_point.get('data_type') == 'calculated':
    ...
    ax.set_title('Figure 7: U-19Pu-10Zr Swelling vs Burnup\\n(EBR-II Assembly X423)',
...
def calculate_validation_metrics(
    ...
    for temp, result in results_dict.items():
        ...
        for data_point in experimental_data:
            if (data_point.get('data_type') == 'calculated' and
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Reproduce Figure 7 - U-19Pu-10Zr Swelling vs Burnup',
...
Examples:
  ...
  python -m gas_swelling.validation.scripts.reproduce_figure7 --output figure7.pdf --no-show
    ...
    parser.add_argument('--output', '-o',
                        type=str,
                        default=None,
                        help='Output filename for the figure (e.g., figure7.png, figure7.pdf)')
    ...
    parser.add_argument('--temperatures',
                        type=str,
                        default='650,750,800',
                        help='Comma-separated list of temperatures in Kelvin (default: 650,750,800)')
    ...
    print("\n" + "=" * 70)
    print("  FIGURE 7 REPRODUCTION - U-19Pu-10Zr SWELLING VS BURNUP")
    print("=" * 70)
    ...
    try:
        experimental_data = get_figure7_data()
    ...
    for temp in temperatures:
        ...
        try:
            result = run_u19pu10zr_simulation(
                temperature=temp,
    ...
    print("\n" + "=" * 70)
    print("  Figure 7 reproduction completed successfully!")
    print("=" * 70)
