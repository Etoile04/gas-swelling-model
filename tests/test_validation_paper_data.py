...
# ============================================================================
# TestU10ZrValidation: Main U-10Zr validation class
# ============================================================================

class TestU10ZrValidation:
    """
    Validation tests for U-10Zr material against Figure 6 paper data.

    These tests verify that the model reproduces results from Figure 6
    ...
    def test_u10zr_model_produces_physically_meaningful_results(self):
        """
        Test that U-10Zr model produces physically meaningful results.
        ...
        simulations and compare against Figure 6 data points using
        the validate_model_results helper function.
        """
        # Create U-10Zr parameters
    ...
    def test_u10zr_validation_helper_function(self):
        """
        Test that the validation helper function works correctly.
        ...
        result = validate_model_results('U-10Zr', 0.5, 0.4, 700, tolerance=0.5)
        assert result['is_valid'], \
            "Value within expected range should be valid"
        assert result['material'] == 'U-10Zr'
        assert result['calculated_swelling'] == 0.5
        assert result['burnup_at_percent'] == 0.4
        assert result['temperature_k'] == 700

        # Test with value outside expected range (should fail)
        result = validate_model_results('U-10Zr', 5.0, 0.4, 700, tolerance=0.5)
        assert not result['is_valid'], \
            "Value outside expected range should be invalid"

        # Test tolerance functionality
        # Expected range at 0.9 at.% is [1.0, 3.0], center is 2.0
        # With 0.5 tolerance, values from 0.5 to 3.5 should be valid
        result1 = validate_model_results('U-10Zr', 2.0, 0.9, 700, tolerance=0.5)
        assert result1['is_valid'], "Center value should be valid"

        result2 = validate_model_results('U-10Zr', 0.6, 0.9, 700, tolerance=0.5)
        assert result2['is_valid'], "Value within tolerance should be valid"

        result3 = validate_model_results('U-10Zr', 4.0, 0.9, 700, tolerance=0.5)
        assert not result3['is_valid'], "Value outside tolerance should be invalid"

    def test_u10zr_material_parameters_match_paper(self):
        """
        Test that U-10Zr material parameters match Table 1 from the paper.
    ...
    def test_u10zr_expected_results_structure(self):
        """
        Test that U-10Zr expected results data structure is properly defined.
    ...
    def test_u10zr_validate_model_results_helper(self):
        """
        Test that the validate_model_results helper function works correctly.

        This test verifies the validation helper that is used to compare
        model results against paper data.
        """
        ...
        result = validate_model_results('U-10Zr', 2.5, 0.9, 700, tolerance=0.5)
        # Expected range at 0.9 at.% is [1.0, 3.0], center is 2.0
        # 2.5 should be valid with 0.5 tolerance
